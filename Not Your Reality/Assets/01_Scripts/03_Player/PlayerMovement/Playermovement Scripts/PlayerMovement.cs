...
public class FirstPersonController : MonoBehaviour
{
   ...
   private void HandleMouseLook()
   {
      ...
      smoothMouseDelta.x = Mathf.SmoothDamp(
         smoothMouseDelta.x,
         targetMouseDelta.x,
         ref currentMouseDelta.x,
         mouseSmoothTime
      );
      smoothMouseDelta.y = Mathf.SmoothDamp(
         smoothMouseDelta.y,
         targetMouseDelta.y,
         ref currentMouseDelta.y,
         mouseSmoothTime
      );
   ...
   private void HandleMovement()
   {
      ...
      if (moveActive)
      {
         ...
         else
         {
            ...
            if (currentVelocity.magnitude < stopThreshold) currentVelocity = Vector3.zero;
         }
      ...
      if (controller.enabled) controller.Move(move * Time.deltaTime);
   }

   private void ApplyGravity()
   {
      if (controller.isGrounded && velocity.y < 0f) velocity.y = -0.05f;
