...
namespace Puzzle.Wordle
{
   public class WordlePuzzle : MonoBehaviour
   {
      ...
      private void UpdateCurrentRow()
      {
         for (int i = 0; i < 5; i++)
         {
            if (i < _currentInput.Length) { _board[_currentGuess][i].SetLetter(_currentInput[i]); }
            else { _board[_currentGuess][i].SetLetter(' '); }
         }
      }
      ...
      private void SubmitGuess()
      {
         ...
         for (int i = 0; i < 5; i++)
         {
            ...
            if (_keyboardButtons.TryGetValue(guessedLetter, out KeyboardButton key))
            {
               ...
               if (color != correctColor && color != presentColor) { key.SetColor(Color.gray); }
            }
         }
