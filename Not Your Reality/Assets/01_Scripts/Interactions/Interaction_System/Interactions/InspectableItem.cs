...
namespace Interactions.Interaction_System.Interactions
{
   public class InspectableItem : InteractableBase
   {
      ...
      private void Init()
      {
         ...
         if (_volume && _volume.profile.TryGet(out _vignette)) _vignette.intensity.value = 0f;
      ...
      public override void OnInteract()
      {
         ...
         if (_inspectRoutine != null) StopCoroutine(_inspectRoutine);
      ...
      private IEnumerator Inspect()
      {
         ...
         if (useVignette) _targetVignette = inspectVignetteIntensity;

         if (lockPlayerUntilEmitterFinished) StartCoroutine(WaitForEmitter());
      }

      private IEnumerator TryRelease()
      {
         if (lockPlayerUntilEmitterFinished && !_audioFinished) yield break;
      ...
      private IEnumerator Release()
      {
         ...
         while (t < duration)
         {
            ...
            _pivot.position = Vector3.Lerp(_pivot.position, _startPos - _startRot * _itemLocalPosInPivot, a);
         ...
         if (bedroomUnlock) bedroomUnlock.AddItem(this);
      ...
      private IEnumerator WaitForEmitter()
      {
         ...
         if (useVignette) _targetVignette = lockedVignetteIntensity;
      ...
      {
         if (!useVignette || !_vignette) return;
